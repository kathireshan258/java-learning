...
//    System.out.println("area: " + area);
  ...
  static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
    if ((width <= 0) || (height <= 0) || (areaPerBucket <= 0) || (extraBuckets < 0)) return -1;

    double area = width * height;
    int noOfBucketsRequired =  getBucketCount(area, areaPerBucket);
    return (noOfBucketsRequired > extraBuckets ? noOfBucketsRequired - extraBuckets : extraBuckets - noOfBucketsRequired);
  }

  static int getBucketCount(double area, double areaPerBucket) {
    if ((area <= 0) || (areaPerBucket <= 0)) return -1;
//    System.out.println(area + "/" + areaPerBucket + ": " + (area / areaPerBucket));
